...
class shurjopay_helper {
     ...
     * @param string   $paymentmodes         whether we are working with the sandbox environment or not
     */
    public function __construct(
        ...
        $this->apiurl = $gatewayurl.'/get_token';
    }
    ...
    public function generate_payment(
        ...
        if ($this->paymentmodes == 'live') {
            $localpc = true;
            $directapiurl = $this->gatewayurl.'/secret-pay?amount='.$cost.'&order_id='.$order_id;
        } else {
            $localpc = false;
            $directapiurl = $this->gatewayurl.'/secret-pay?amount='.$cost.'&order_id='.$order_id;
        }
    ...
    public function varify_payment (string $token, string $order_id) {
        ...
        if ($this->paymentmodes == 'live') {
            $localpc = true;
            $directapiurl = $this->gatewayurl.'/verification';
        } else {
            $localpc = false;
            $directapiurl = $this->gatewayurl.'/verification';
        }
