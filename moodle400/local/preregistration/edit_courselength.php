...
use local_preregistration\form\courselength_form;

require('../../config.php');
require_once('./lib.php');
require_once($CFG->dirroot.'/local/preregistration/classes/form/courselength_form.php');
try {
    require_login();
...
$PAGE->set_url('/local/preregistration/edit_courselength.php');
$PAGE->set_context(\context_system::instance());
$PAGE->set_title(get_string('edit_courselength', 'local_preregistration'));
$PAGE->set_heading(get_string('edit_courselength', 'local_preregistration'));
...
if ($dataid) {
    $actionurl = new moodle_url('/local/preregistration/edit_courselength.php?batchid=' . $batchid . '&dataid=' . $dataid);
    $mform = new courselength_form($actionurl);
    
} else {
    $actionurl = new moodle_url('/local/preregistration/edit_courselength.php?batchid=' . $batchid);
    $mform = new courselength_form($actionurl);
}
...
if($dataid) {
    $courselength = $DB->get_record('local_preregistration_data', array('id' => $dataid));
    $mform->set_data($courselength);
}
