...
use local_preregistration\form\programcost_form;

require('../../config.php');
require_once('./lib.php');
require_once($CFG->dirroot.'/local/preregistration/classes/form/programcost_form.php');
try {
    require_login();
...
$PAGE->set_url('/local/preregistration/edit_cost.php');
$PAGE->set_context(\context_system::instance());
$PAGE->set_title(get_string('edit_cost', 'local_preregistration'));
$PAGE->set_heading(get_string('edit_cost', 'local_preregistration'));
...
if ($dataid) {
    $actionurl = new moodle_url('/local/preregistration/edit_cost.php?batchid=' . $batchid . '&dataid=' . $dataid);
    $mform = new programcost_form($actionurl);
    
} else {
    $actionurl = new moodle_url('/local/preregistration/edit_cost.php?batchid=' . $batchid);
    $mform = new programcost_form($actionurl);
}
...
if($dataid) {
    $cost = $DB->get_record('local_preregistration_data', array('id' => $dataid));
    $mform->set_data($cost);
}
