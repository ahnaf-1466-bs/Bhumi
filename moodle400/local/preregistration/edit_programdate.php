...
use local_preregistration\form\programdate_form;

require('../../config.php');
require_once('./lib.php');
require_once($CFG->dirroot.'/local/preregistration/classes/form/programdate_form.php');
try {
    require_login();
...
$PAGE->set_url('/local/preregistration/edit_programdate.php');
$PAGE->set_context(\context_system::instance());
$PAGE->set_title(get_string('edit_programdate', 'local_preregistration'));
$PAGE->set_heading(get_string('edit_programdate', 'local_preregistration'));
...
if ($dataid) {
    $actionurl = new moodle_url('/local/preregistration/edit_programdate.php?batchid=' . $batchid . '&dataid=' . $dataid);
    $mform = new programdate_form($actionurl);
    
} else {
    $actionurl = new moodle_url('/local/preregistration/edit_programdate.php?batchid=' . $batchid);
    $mform = new programdate_form($actionurl);
}
...
if($dataid) {
    $programdate = $DB->get_record('local_preregistration_data', array('id' => $dataid));
    $mform->set_data($programdate);
}
