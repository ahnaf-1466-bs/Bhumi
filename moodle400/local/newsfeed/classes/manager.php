...
class manager {
    ...
    public function create_message(array $newsimage, array $content): bool
    {
        ...
        try {
            return $DB->insert_record('newsfeed_newsdetails', $record_to_insert, false);
    ...
    public function get_messages(int $userid): array
    {
        ...
        $sql = "SELECT lm.id, lm.messagetext, lm.messagetype 
            FROM {local_newsfeed} lm 
            LEFT OUTER JOIN {local_newsfeed_read} lmr ON lm.id = lmr.newsid AND lmr.userid = :userid 
    ...
    public function get_all_messages(): array {
        global $DB;
        return $DB->get_records('local_newsfeed');
    }
    ...
    public function mark_message_read(int $message_id, int $userid): bool
    {
        ...
        try {
            return $DB->insert_record('local_newsfeed_read', $read_record, false);
        } catch (dml_exception $e) {
            return false;
        }
    }
    ...
    {
        global $DB;
        return $DB->get_record('newsfeed_newsdetails', ['id' => $newsid]);
    }
    ...
    public function update_message(int $newsid, int $course, string $newsimage, string $content)
    {
        ...
        return $DB->update_record('newsfeed_newsdetails', $object, false, ['id' => 2]);
    }
    ...
    public function update_messages(array $newsids, $type): bool
    {
        ...
        return $DB->set_field_select('local_newsfeed', 'messagetype', $type, "id $ids", $params);
    }
    ...
    public function delete_message($newsid)
    {
        ...
        $deletedMessage = $DB->delete_records('local_newsfeed', ['id' => $newsid]);
        $deletedRead = $DB->delete_records('local_newsfeed_read', ['newsid' => $newsid]);
    ...
    public function delete_messages($newsids)
    {
        ...
        $transaction = $DB->start_delegated_transaction();
        list($ids, $params) = $DB->get_in_or_equal($newsids);
        $deletedMessages = $DB->delete_records_select('local_newsfeed', "id $ids", $params);
        $deletedReads = $DB->delete_records_select('local_newsfeed_read', "newsid $ids", $params);
